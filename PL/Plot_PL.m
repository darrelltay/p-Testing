function Plot_PL(Result,XLabelName)
...
Alpha_Unadj = Result.Unadjusted.Parameter.Alpha;
...
Alpha_Adj = Result.Adjusted.Parameter.Alpha;
...
WoCDF = EmpCDF(sum(Sample<Xmin_Unadj))*(WoSS/Xmin_Unadj).^(1-Alpha_Unadj);
%%%% Adjusted 
WSS = linspace(Xmin_Adj,max(Sample),50);
WCDF = EmpCDF(sum(Sample<Xmin_Adj))*(WSS/Xmin_Adj).^(1-Alpha_Adj);
UpperBound = 1-(max(Sample)./Xmin_Adj).^(1-Alpha_Adj);
...
%%%% Xtick and Ytick
XTick =10.^(((floor(log(prctile(Sample,50))/log(10)))):(ceil(log(max(Sample))/log(10))));
YTick = 10.^(floor((log(EmpCDF_Adj(1,end))/log(10))):ceil((log(1)/log(10))));
TextPostionX = 10.^linspace((log(XTick(1,1))/log(10)),(log(XTick(1,end))/log(10)),190);
